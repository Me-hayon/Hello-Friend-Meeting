...
public class SensService {
	...
	public int makeBody(String toTel) {
		...
		bodyJson.put("contentType", ""); // 메시지 내용 Type (AD | COMM) * AD: 광고용, COMM: 일반용 (default: COMM) * 광고용 메시지 발송 시
											// 불법 스팸 방지를 위한 정보통신망법 (제 50조)가 적용됩니다.
		bodyJson.put("countryCode", "82"); // 국가 전화번호
		bodyJson.put("from", "01033440781"); // 발신번호 * 사전에 인증/등록된 번호만 사용할 수 있습니다.
		bodyJson.put("subject", ""); // 메시지 제목 * LMS Type에서만 사용할 수 있습니다.

		// 이 밑의 content랑 위의 content랑 차이점이 뭔지 모르겠음. 문자는 위에 있는 toJson의 content로 옴.
		bodyJson.put("content", "bodyJson 안녕하세요^^"); // 메시지 내용 * Type별로 최대 byte 제한이 다릅니다.* SMS: 80byte / LMS: 2000byte
